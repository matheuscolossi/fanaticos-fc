...
function aggregateItems(items) {
  ...
  for (const item of parseItems(items)) {
    ...
    const key = `${productId}:${tamanho || ''}`;
    const current = totals.get(key) || { productId, tamanho, quantity: 0 };
  ...
  return [...totals.values()].sort(
    (a, b) => a.productId - b.productId || String(a.tamanho).localeCompare(String(b.tamanho))
  );
}
...
async function getVariantContext(item, db) {
  ...
  if (Number(count?.total || 0) === 0) return { usesVariants: false, variant: null };
  if (!item.tamanho) {
  ...
  return { usesVariants: true, variant };
}
